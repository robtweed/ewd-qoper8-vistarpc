...
function runRPC(params, session, manageSymbolTable) {
  ...
  //console.log('vistarpc.session: ' + JSON.stringify(session));
  console.log('manageSymbolTable: ' + manageSymbolTable);

  var data;
  var context = '';

  // SMH: This is incorrect. Should never set a default Context. VISTA assigns one for us on first
  // log-on. This breaks some logging in behavior.
  ...
  */
  ...
  /* SMH: This is incorrect. User must select division via XUS DIVISION GET/SET.
  ...
  if (gloRef.$('ERRORTRAP').exists) {
    ...
    /* SMH: Had to add the typeof thing b/c this crashed on numeric returns (don't have indexOf) */
      ...
      if (results.type === 'GLOBAL ARRAY') {
        ...
        /*
        var global = gloRef.$('result').$('value')._value;
...
function convGRef2NRef(gloRef)
{
  ...
  // TODO: (maybe never) -- Exclude commas inside of strings
